...
proof (rule total_hyper_tripleI)
  show "\<Turnstile> {P} stmt.If (Assume b ;; C1) (Assume (lnot b) ;; C2) {join Q1 Q2}"
    apply (intro if_rule seq_rule)
    using assms(1) apply simp
    using assms(3) total_hyper_triple_def apply blast
    using assms(2) apply simp
    using assms(4) total_hyper_triple_def by blast
  fix \<phi> S
  assume asm0: "P S \<and> \<phi> \<in> S"
  show "\<exists>\<sigma>'. \<langle>stmt.If (Assume b ;; C1) (Assume (lnot b) ;; C2), snd \<phi>\<rangle> \<rightarrow> \<sigma>'"
  proof (cases "b (snd \<phi>)")
    case True
    then have "Pt (sem (Assume b) S) \<and> \<phi> \<in> sem (Assume b) S"
      by (metis SemAssume asm0 assms(1) hyper_hoare_tripleE in_semI)
    then have "\<exists>\<sigma>'. \<langle>C1, snd \<phi>\<rangle> \<rightarrow> \<sigma>'"
      using assms(3) total_hyper_tripleE by blast
    then show ?thesis
      by (meson SemAssume SemIf1 SemSeq True)
  next
    case False
    then have "Pf (sem (Assume (lnot b)) S) \<and> \<phi> \<in> sem (Assume (lnot b)) S"
      by (metis (mono_tags, opaque_lifting) SemAssume asm0 assms(2) hyper_hoare_triple_def in_semI lnot_def)
    then have "\<exists>\<sigma>'. \<langle>C2, snd \<phi>\<rangle> \<rightarrow> \<sigma>'"
      using assms(4) total_hyper_tripleE by blast
    then show ?thesis
      by (metis False SemAssume SemIf2 SemSeq lnot_def)
  qed
qed
